...
export function registerThinTools(server: McpServer, client: DraftboardClient): void {
  ...
  server.registerTool(
    "get_target_connections",
    {
      ...
      description:
        "List the warm-intro connections for one target: each connector's profile, relationship `score` (0-100, `rank` in the outcome tools), `scoreDetails` (the human-readable shared history, far more often present than the structured fields (it is the model's own prose), though it too is omitted when there is nothing to say), and the team members (`owners`) whose network the connection comes from. Filter by `ownerIds` to see paths through specific teammates. Paginated. " +
        ...
        "BOTH KEYS ARE ABSENT WHEN EMPTY — the key is simply not in the JSON, it is never `[]` — so read them as `connection.relationships ?? []`. Empty is the MAJORITY case (ranks scored before the structured model shipped carry none, and there is no backfill): absence means \"we hold no structured signal for this pair\", NOT \"these two have no relationship\". Never drop or downrank a connector for it; `scoreDetails` remains the fallback. " +
        "The two fields are also INDEPENDENT: a connection whose only signal is shared contacts gets a `relationshipDetails` record and NO `relationships` entry, and an older rank can carry `relationships` with no records. Never derive, gate or index-align one from the other or from `scoreDetails`.",
      inputSchema: {
